...
export const GearSection = () => {
    ...
    // Class state
    const [charClass, setCharClass] = useState(user?.profile?.char_class || '');
    const [isSavingClass, setIsSavingClass] = useState(false);
    const [isClassModalOpen, setIsClassModalOpen] = useState(false);

    // Manual stats state
    const [attack, setAttack] = useState(user?.profile?.attack ?? 0);
    const [awakeningAttack, setAwakeningAttack] = useState(user?.profile?.awakening_attack ?? 0);
    const [defense, setDefense] = useState(user?.profile?.defense ?? 0);
    ...
    useEffect(() => {
        if (user?.profile) {
            setCharClass(user.profile.char_class || '');
            setAttack(user.profile.attack ?? 0);
            setAwakeningAttack(user.profile.awakening_attack ?? 0);
            setDefense(user.profile.defense ?? 0);
            setGarmothUrl(user.profile.garmoth_url || '');
        }
    }, [user]);
    ...
    const currentClassObj = BDO_CLASSES.find(
        c => c.id.toLowerCase() === charClass.toLowerCase() || c.nameEn.toLowerCase() === charClass.toLowerCase()
    );
    ...
    const confirmSwitchToManual = async () => {
        ...
        try {
            ...
            setUser(updatedUser);
            toast.info('Режим изменен на ручное заполнение. Garmoth-ссылка удалена.');
        } catch (err: any) {
    ...
    const handleClassChange = async (newClass: string) => {
        ...
        try {
            const updatedUser = await authApi.updateProfile({ char_class: newClass });
            setUser(updatedUser);
            toast.success('Класс персонажа обновлен');
        } catch (err: any) {
            toast.error(err.response?.data?.message || 'Ошибка при изменении класса');
        } finally {
    ...
    const handleSaveManual = async () => {
        ...
        try {
            const updatedUser = await authApi.updateProfile({
                ...
                awakening_attack: Number(awakeningAttack),
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h2 className="text-xl font-bold text-zinc-100 tracking-tight">Управление данными экипировки</h2>
                </div>
                <div className={cn(
            ...
            {/* ВЕРХНИЙ БЛОК: Выбор класса */}
            <div className="bg-zinc-950/40 border border-white/[0.06] rounded-2xl p-5 md:p-6 space-y-3">
                <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                    Игровой класс персонажа
                </label>

                <div
                    onClick={() => !isPending && setIsClassModalOpen(true)}
                    className={cn(
                        "group relative bg-zinc-900 border rounded-xl px-4 py-3.5 flex items-center justify-between transition-all cursor-pointer",
                        isPending ? "opacity-50 cursor-not-allowed border-white/[0.08]" : "border-white/[0.08] hover:border-violet-500/50 hover:bg-zinc-900/90 active:scale-[0.99]"
                    )}
                >
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-violet-600/10 border border-violet-500/20 flex items-center justify-center text-violet-400 font-bold text-sm shrink-0">
                            {currentClassObj?.nameRu ? currentClassObj.nameRu[0] : '?'}
                        </div>
                        <div>
                            <p className="text-sm font-bold text-zinc-100 group-hover:text-violet-300 transition-colors">
                                {currentClassObj ? `${currentClassObj.nameRu} (${currentClassObj.nameEn})` : (charClass || 'Класс не выбран')}
                            </p>

                        </div>
                    </div>

                    <div className="flex items-center gap-2 text-zinc-400 group-hover:text-violet-400 transition-colors">
                        {isSavingClass ? (
                            <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                        ) : (
                            <span className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-zinc-800 group-hover:bg-violet-600/20 border border-white/5 transition-all text-zinc-300 group-hover:text-violet-200">
                                {charClass ? 'Сменить класс' : 'Выбрать класс'}
                            </span>
                        )}
                    </div>
                </div>
            ...
            {/* СРЕДНИЙ БЛОК: Переключатель источника экипировки */}
            <div className="space-y-4">
                            ...
                            <div className="flex flex-col sm:flex-row gap-3">
                                ...
                                <div className="flex gap-2 w-full sm:w-auto">
                                    <button
                                        ...
                                        className="flex-1 sm:flex-initial px-6 py-3 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white font-semibold text-xs rounded-xl transition-all flex items-center justify-center gap-2 text-center shrink-0 cursor-pointer"
                                    >
                                        {isSavingGarmoth ? (
                                            <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                                        ) : (
                                    ...
                                    {user?.profile?.garmoth_url && (
                                        <button
                                            ...
                                            className="flex-1 sm:flex-initial px-5 py-3 bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-zinc-200 text-xs font-semibold rounded-xl border border-white/5 transition-all flex items-center justify-center gap-1.5 text-center shrink-0 cursor-pointer"
                ...
                {gearSource === 'manual' && (
                    <div className="space-y-6 animate-in fade-in duration-300">
                        {/* Manual Stats Input */}
                        <div className="bg-zinc-950/40 border border-white/[0.06] rounded-2xl p-5 md:p-6 space-y-4">
                            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Ручной ввод</p>
                            <div className="grid grid-cols-3 gap-3 sm:gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-semibold uppercase tracking-wider text-zinc-500 block text-center">AP</label>
                                    <input
                                        type="number"
                                        value={attack}
                                        onChange={(e) => setAttack(Number(e.target.value))}
                                        disabled={isPending}
                                        className="w-full bg-zinc-900 border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-zinc-100 text-center outline-none focus:border-violet-500/50 transition-all disabled:opacity-50"
                                    />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-semibold uppercase tracking-wider text-zinc-500 block text-center">AAP</label>
                                    <input
                                        type="number"
                                        value={awakeningAttack}
                                        onChange={(e) => setAwakeningAttack(Number(e.target.value))}
                                        disabled={isPending}
                                        className="w-full bg-zinc-900 border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-zinc-100 text-center outline-none focus:border-violet-500/50 transition-all disabled:opacity-50"
                                    />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-semibold uppercase tracking-wider text-zinc-500 block text-center">DP</label>
                                    <input
                                        type="number"
                                        value={defense}
                        ...
                        {/* Actions Row in ONE Line */}
                        <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
                            <button
                                onClick={handleSaveManual}
                                disabled={isSavingManual || isPending}
                                className="w-full sm:w-1/2 py-3.5 bg-zinc-800 hover:bg-zinc-700 border border-white/10 text-white font-bold text-sm rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                            >
                                {isSavingManual ? (
                                    <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" />
                                ) : (
                            ...
                            {isPending ? (
                                <div className="w-full sm:w-1/2 bg-zinc-950/40 border border-white/[0.06] rounded-xl p-3 text-center text-zinc-400 text-xs">
                                    На рассмотрении
                                </div>
                            ) : (
                                <button
                                    ...
                                    className="w-full sm:w-1/2 flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:bg-zinc-800 disabled:text-zinc-600 text-white px-6 py-3.5 rounded-xl font-bold text-sm shadow-lg shadow-violet-900/30 transition-all duration-200 active:scale-[0.98] cursor-pointer disabled:cursor-not-allowed text-center"
                                >
                                    {isUploading ? (
            ...
            >
                <div className="space-y-4">
                    <p className="text-xs text-zinc-300 leading-relaxed">
                        Вы переключаетесь на ручной режим заполнения. Ссылка на ваш профиль в Garmoth.com будет отвязана и удалена.
                    </p>
